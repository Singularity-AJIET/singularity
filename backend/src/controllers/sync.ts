 ...
 */
export async function syncBatchScans(req: Request, res: Response, next: NextFunction) {
  try {
    const items = req.body as ScanBatchItem[];
      ...
      try {
        ...
        if (isUnique) {
          ...
          results.push({
            scan_id: scanId,
            status: "ERROR",
            message: err instanceof Error ? (err as Error).message : "Database write failed."
          });
        }
      }
    }
